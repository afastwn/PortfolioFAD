...
@section('content')
    ...
    <div class="overflow-x-auto">
        <table id="studentsTable" class="min-w-full bg-white border border-gray-200 rounded-xl shadow-sm">
            <thead class="bg-[#f9fafb] border-b">
                <tr>
                    <th class="py-3 px-5 text-left font-semibold text-gray-600 text-sm">Student ID</th>
            ...
            <tbody>
                @php
                    $students = \App\Models\User::where('role', 'mahasiswa')->orderBy('nim')->get();
                @endphp

                {{-- Jangan pakai @forelse/@empty agar tidak ada <tr colspan> yang bikin DataTables warning --}}
                @foreach ($students as $student)
                    <tr class="{{ $loop->even ? 'bg-[#fafaff]' : 'bg-[#f8f6ff]' }} hover:bg-gray-50 transition">
                        <td class="py-3 px-5 text-gray-700">{{ $student->nim }}</td>
                        <td class="py-3 px-5 text-gray-700">{{ $student->name_asli }}</td>
                        <td class="py-3 px-5">
                            <div class="flex items-center space-x-3">
                                ...
                                <form action="{{ route('admin.user.destroy', $student->id) }}" method="POST"
                                    onsubmit="return confirm('Hapus akun {{ $student->name_asli }} ({{ $student->nim ?? $student->nip }})?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
        <div class="bg-white rounded-xl shadow-lg w-96 p-6">
            ...
            <form id="addDosenForm" method="POST" action="{{ route('admin.students.store') }}">
                @csrf
                <div class="mb-4">
                    <label for="nim" class="block text-sm font-semibold mb-1">Student ID</label>
    ...
    <!-- DataTables (JS only, supaya desain tabel tetap Tailwind) -->
        ...
        $(function() {
            const table = $('#studentsTable').DataTable({
                ...
                initComplete: function(settings) {
                    // Jika tidak ada data sama sekali, tampilkan baris info secara manual
    ...
    <!-- (Opsional) Pertahankan striping baris sesuai desain -->
    <style>
